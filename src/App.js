...
function App() {
  ...
  return (
    <div className="w-screen h-screen bg-blue-300 p-24">
      <div
        className="container mx-auto border-4 overflow-y-scroll h-full flex"
        style={{ borderColor: "#A38450" }}
      >
        <div
          className="w-2/3 flex flex-col text-white"
          style={{ backgroundColor: "#071A20" }}
        >
          <input
            onChange={(e) => {
              searchItem(e.target.value);
            }}
            className="m-2 appearance-none border-2 rounded py-2 px-3 text-gray-700 focus:outline-none focus:shadow-outline"
            type="text"
            ...
            {items.length > 0 ? (
              items.map((item, index) => (
                <div
                  ...
                  className="border-2 m-2 flex flex-col items-center cursor-pointer hover:border-fuchsia-700 border-gray-700"
                  key={index}
                >
                  <img
                    src={
                      "http://ddragon.leagueoflegends.com/cdn/13.13.1/img/item/" +
                      item.image.full
                    }
                    width="64px"
                    height="64px"
